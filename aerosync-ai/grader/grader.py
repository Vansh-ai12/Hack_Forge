...
Score formula (per difficulty):
    score = completion_weight  * completion_ratio
           + efficiency_weight * efficiency_ratio
           + safety_weight     * safety_factor
           + priority_weight   * priority_score
           + drone_weight      * drone_quality_score

All sub-scores in [0.0, 1.0]. Final clipped to [0.0, 1.0].
"""
...
# Penalty magnitudes (deductions from a 1.0 sub-score)
...
def _drone_quality_score(state: Dict[str, Any]) -> float:
    ...
    # Stability: below 0.7 average is bad
    if avg_stab_score < 0.7:
        deduction += 0.1 * (0.7 - avg_stab_score) / 0.7

    # Delivery precision: failed drops vs total attempts
